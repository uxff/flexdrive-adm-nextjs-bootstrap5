...
// 作者：南殇
// 链接：https://juejin.cn/post/7188077505184546871
// 来源：稀土掘金
...
interface PageHelperProps {
  ...
  // tapSelectOnePage: (page: number) => void;
}

const PageHelper: React.FC<PageHelperProps> = ({allPage, currentPage, handleSearch, loading}) => {
  ...
  const INTERTALS = 6;
  const [nums, setNums] = useState<number[]>([]); // 显示的页码数组
  ...
  const changePage = (total: number, cur: number) => {
    const numsRes: number[] = drawPage(total, cur);
    setCurrent(cur);
    setNums(numsRes);
    handleSearch(cur);
  };
   ...
   * @param {number} cur
   */
  const drawPage = (total: number, cur: number):any[] => {
   ...
   */
  const around = (total: number, curPage: number) => {
  ...
  return (
    <>
      ...
      {
        nums.map((item, index) => {
          const isMore = item.toString() === MORETXT;
          const isChoosed = item === current;
          const cssName = 'page-item' + (isChoosed ? ' active' : '');
          return <li key={index}
